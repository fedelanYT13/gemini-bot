...
♡ Preparando tu archivo, espera un momento...
`.trim()
    ...
    if (["play", "yta", "ytmp3", "playaudio"].includes(command)) {
      ...
      if (!audioData) {
        await conn.sendMessage(m.chat, { react: { text: "❌", key: m.key}})
        return conn.reply(m.chat, "✧︎ No se pudo descargar el audio. Intenta más tarde.", m)
}
      ...
      if (!videoData) {
        await conn.sendMessage(m.chat, { react: { text: "❌", key: m.key}})
        return conn.reply(m.chat, "✧︎ No se pudo descargar el video. Intenta más tarde.", m)
}
